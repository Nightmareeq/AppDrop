...
bot = telebot.TeleBot('8447166560:AAHYsL9aAF1MJkRkI4yTrVQA4zY-eIOawvI')
...
def receive_screenshot(message):
    ...
    if message.content_type == 'photo':
        try:
            ...
            bot.send_photo(chat_id=7495969992, photo=file_id, caption=caption)
...
def get_apps(message):
    ...
    bot.send_message(chat_id=7495969992, text=f"Модель айфона: {model}\nРегион/Страна: {region}\nПришла за: {apps}\nЮзер: @{message.from_user.username}\nID-типа: {message.from_user.id}")
    ...
    bot.send_photo(user_id, 'https://postimg.cc/68hLT5Wh')
...
def helper(call):
    if call.data == 'help':
        bot.send_message(call.message.chat.id, 'Если у тебя возникли вопросы или нужна помощь, не стесняйся — свяжись с @karolynaas, и он(а) обязательно поможет разобраться.')

@bot.message_handler(commands=['block'])
def finish_cmd(message):
    if message.chat.id != 7495969992:
        return
